...
import {
  AdminEditInfoReleaseApi,
  GetFeaturingArtistApi,
  GetGenreApi,
  GetPrimaryArtistApi,
  GetReleaseInfoApi,
  GetReleaseInfoByIdApi,
  ReleaseInfoPostApi,
  UserDataApi,
...
export default function EditReleasePopUp({
  id,
  userId,
}: {
  id: any;
  ...
  const {
    ...
    isFetching,
    refetch,
  } = GetReleaseInfoByIdApi(id);
  const { mutate: ReleaseInfoEdit, isLoading: isLoadingReleaseInfoEdit } =
    AdminEditInfoReleaseApi(id, refetch, setIsOpen);
  ...
  React.useEffect(() => {
    if (!isLoading) {
      setSelectedItems(getReleaseInfo?.data?.data?.ReleaseType);
      setValue("ReleaseType", getReleaseInfo?.data?.data?.ReleaseType);
  ...
  const onSubmit = async (data: ReleaseInfoDto) => {
                  ...
                  <div className="mt-2">
                    <div className="flex flex-col items-center mt-4 space-y-4">
                      <FileUpload
                        file={file}
                        setFile={setFile}
                        previewFile={getReleaseInfo?.data?.data?.ImageDocument}
                        preview={preview}
                        setPreview={setPreview}
                      />
                      <div className="text-left">
                        <h6 className="font-bold text-center text-teal-500">
                          Artwork Guidelines
                        </h6>
                        <ul className="list-disc list-inside text-xs">
                          ...
                          <li>
                            For optimal results, ensure the image dimensions are
                            N x N pixels.(eg :1000 x 1000 , 2000 x 2000)
                          </li>
                        </ul>
                      </div>
                    </div>
                    ...
                    <div className="w-full mb-2">
                      ...
                      <div className="flex flex-wrap items-center gap-6 px-4 py-1">
                        {items.map((item, index) => (
                          <p
                            key={index}
                            className={`text-xs sm:text-sm p-2 font-semibold border-2 ${
                              selectedItems === item
                    ...
                    <div className="w-full mb-2">
                      <Label
                        text="Featuring Artist"
                        htmlFor="grid-FeaturingArtist"
                        required={false}
                      />
                      <div className="flex gap-2 items-center">
                        <SelectFeatureArtist
